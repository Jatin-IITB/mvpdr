...
def build_visual_prototypes(cfg, clip_model, train_loader_cache, n_cls, n_clt):
    if cfg['load_cache'] == False:
        ...
        for k in feature_dict.keys():
            ...
            # Adaptive n_clt based on available samples
            # K-Means requires n_clusters <= n_samples
            adaptive_n_clt = min(n_clt, max(1, n_samples))

            if adaptive_n_clt < n_clt:
                classes_with_few_samples.append((k, n_samples, adaptive_n_clt))

            if n_samples == 1:
                # Only 1 sample, use it directly as prototype
                item = tensors
                warnings.warn(f'Class {k} has only 1 sample. Using it as single prototype.')
            elif adaptive_n_clt == 1:
                # Use mean as single prototype
                item = tensors.mean(dim=0, keepdim=True)
            else:
                # Perform K-Means clustering
                im_arr = tensors.detach().cpu().numpy()
                kmeans = KMeans(n_clusters=adaptive_n_clt, random_state=0, n_init=10)
                kmeans.fit(im_arr)
                item = transforms.ToTensor()(kmeans.cluster_centers_)
                item = item.squeeze(dim=0)

            features_list.append(item)
            labels = [k] * item.shape[0]  # Use actual number of prototypes created
            label_list += labels

        # Print statistics
        print(f'\nVisual Prototype Statistics:')
        print(f'  Total classes: {n_cls}')
        print(f'  Samples per class range: {min_samples} to {max_samples}')
        print(f'  Requested prototypes per class: {n_clt}')

        if classes_with_few_samples:
            print(f'\n  WARNING:  Warning: {len(classes_with_few_samples)} classes have fewer samples than requested prototypes!')
            for class_idx, n_samples, adaptive_n_clt in classes_with_few_samples[:10]:
                print(f'      Class {class_idx}: {n_samples} samples to using {adaptive_n_clt} prototypes')
            if len(classes_with_few_samples) > 10:
                print(f'      ... and {len(classes_with_few_samples) - 10} more classes')
            print(f'\n  TIP: Recommended: Use --n_clt {min_samples} or lower for this dataset')
        ...
        print(f'\n Created {cache_keys.shape[1]} total visual prototypes across {n_cls} classes')
